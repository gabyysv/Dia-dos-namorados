...
(function initBokeh() {
  ...
  const COLS   = [
    [255,60,160], [255,120,200], [220,0,100],
    [255,180,220], [180,0,80],  [255,80,180],
  ];
  let W, H, circles;
  ...
  (function draw() {
    ...
    circles.forEach(c => {
      ...
      c.x+=c.vx; c.y+=c.vy;
      if(c.x<-r*2) c.x=W+r; if(c.x>W+r*2) c.x=-r;
      if(c.y<-r*2) c.y=H+r; if(c.y>H+r*2) c.y=-r;
    });
...
/* ─────────────────────────────────────────
   CORAÇÕES OUTLINE grandes no fundo
───────────────────────────────────────── */
(function initOutline() {
  const el = document.getElementById('outlineHearts');
  for(let i=0;i<22;i++){
    const s = document.createElement('span');
    s.className='oh'; s.textContent='♥';
...
/* ═══════════════════════════════════════════════════
   MÁQUINA DE ESTADOS  (gift → letter → card)
═══════════════════════════════════════════════════ */
...
(function initGift() {
  ...
  function onGiftClick() {
    ...
    // 2) Laço se solta
    setTimeout(() => bow.classList.add('bow-release'), 200);

    // 3) Corações explodem da caixa
    setTimeout(burstFromBox, 350);

    // 4) Tampa voa
    setTimeout(() => lid.classList.add('lid-fly'), 500);

    // 5) Hint muda
    setTimeout(() => {
      hint.textContent = '✨ Veja o que tem dentro…';
      hint.style.animation = 'none';
    }, 700);

    // 6) Vai para cena do envelope
    setTimeout(goToLetter, 1700);
  }
...
/* Explosão de corações saindo da caixa */
function burstFromBox() {
  const SYMS = ['❤','💕','💗','♥','🌸','✨','💝'];
  const rect = $('giftWrap').getBoundingClientRect();
  const cx   = rect.left + rect.width  / 2;
  const cy   = rect.top  + rect.height / 4; // boca da caixa
  ...
  for (let i = 0; i < 22; i++) {
    ...
    const angle = (Math.PI * 2 / 22) * i + (Math.random() * .5 - .25);
...
function initLetter() {
  ...
  // Balanço contínuo após entrada
  setTimeout(() => wrap.classList.add('envelope-idle'), 950);
  ...
  function onEnvelopeClick() {
    ...
    // 3) Envelope dispara para cima
    setTimeout(() => wrap.classList.add('envelope-exit'), 650);

    // 4) Vai para cena do cartão
    setTimeout(goToCard, 1200);
  }
...
/* Corações caindo no stage */
function initFallingHearts() {
  const container = $('fallingHearts');
  const stage     = document.querySelector('.stage');
  const SYMS      = ['❤','♥','💕','🌸'];

  function h() { return stage ? stage.offsetHeight : 560; }
  ...
  for(let i=0;i<20;i++){
    ...
    const del = -(Math.random()*dur);
    const hue = Math.random()<.5 ? 340+Math.random()*20 : 0;
    el.style.cssText=`
      font-size:${sz}px;
      left:${Math.random()*100}%;
      color:hsl(${hue},${80+Math.random()*20}%,${52+Math.random()*18}%);
      --sh:${h()}px;
      animation-duration:${dur}s;
      animation-delay:${del}s;
    `;
  ...
  window.addEventListener('resize',()=>{
    container.querySelectorAll('.fh').forEach(e=>e.style.setProperty('--sh',h()+'px'));
  });
}
...
/* Linhas da mensagem entram em cascata */
function revealCardLines() {
  const msg = document.querySelector('.card-msg');
  if (!msg) return;

  // Quebra o texto em spans individuais por <br>
  const raw  = msg.innerHTML;
  const lines= raw.split('<br>').filter(l=>l.trim());
  msg.innerHTML = lines.map(l=>`<span>${l}</span>`).join('');
  msg.classList.add('reveal');

  const spans = msg.querySelectorAll('span');
  spans.forEach((sp, i) => {
    setTimeout(() => sp.classList.add('show'), 600 + i * 320);
  });
}

/* Hover 3-D suave no cartão */
(function initCardHover() {
  ...
  card.addEventListener('mousemove', e => {
    ...
    if (!raf) raf = requestAnimationFrame(() => {
      card.style.transform = `perspective(600px) rotateY(${tx}deg) rotateX(${ty}deg) scale(1.02)`;
      raf = null;
    });
  });
  ...
  card.addEventListener('touchend', ()=>{
    ...
    setTimeout(()=>{ card.style.transition=''; },330);
  },{passive:true});
})();
